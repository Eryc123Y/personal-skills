> **Compatibility note:** This is a legacy reference. The current [skill entrypoint](../SKILL.md), user request, and vault manual control scope, routes, and formatting. Read-only requests remain read-only; examples here do not require extra notes, automatic saving, setup, or global propagation.
